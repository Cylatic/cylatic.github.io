...
The workflow scans free public cybersecurity feeds every hour, scores the news
locally, and only calls Gemini when there is a sufficiently important, unseen
story. A daily publication ceiling prevents low-value content flooding the site.
"""
...
def fetch(url):
    req = urllib.request.Request(url, headers={"User-Agent": "Cylatic-ResearchBot/3.0"})
...
def score(item):
    ...
    keyword_score = sum(value for keyword, value in KEYWORDS.items() if keyword in text)
    source_score = item["weight"] * 10
    fresh_score = 3 if item["published"] else 0
    return source_score + keyword_score + fresh_score
...
def select_candidate(items, existing_text):
    ...
    for item in items:
        ...
        if len(normalized) < 15:
            continue
        if normalized in existing or item["link"].lower() in existing:
            continue
...
def update_index(post, date):
    ...
    index_path.write_text(text[:marker.start()] + replacement + text[marker.end():], encoding="utf-8")
...
def update_sitemap(post, date):
    ...
    sitemap_path.write_text(sitemap.replace("</urlset>", addition + "</urlset>"), encoding="utf-8")
...
def main():
    ...
    post["slug"] = slug
    if not post.get("title") or not post.get("body_html"):
        raise RuntimeError("Gemini returned incomplete article data")

    output_dir = ROOT / "blog" / f"{today:%Y}" / f"{today:%m}" / f"{today:%d}"
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / f"{slug}.html"
    if output_file.exists():
        print("Article already exists; no duplicate published.")
        return

    output_file.write_text(article_page(post, today), encoding="utf-8")
    update_index(post, today)
    update_sitemap(post, today)
    print(f"Published one high-value cybersecurity research article: {output_file.relative_to(ROOT)}")
