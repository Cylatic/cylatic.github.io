...
KEYWORDS = {
    "zero-day": 9, "0-day": 9, "actively exploited": 10, "critical vulnerability": 8,
    "ransomware": 8, "exploitation": 6, "breach": 5, "supply chain": 7,
    "identity": 4, "cloud": 3, "microsoft": 3, "ai security": 6, "apt": 6,
    "cisa": 5, "critical infrastructure": 7, "malware": 5, "phishing": 4,
    "credential": 4, "remote access": 5, "zero trust": 4, "kubernetes": 4,
}
...
def parse_feed(name, url, weight):
    ...
    for item in items[:35]:
        ...
        ln = item.find("link")
        link = (ln.text or "").strip() if ln is not None and ln.text else ""
        if not link:
            ln = item.find("{http://www.w3.org/2005/Atom}link")
            link = ln.attrib.get("href", "") if ln is not None else ""
        ...
        if title and link:
            out.append({"source": name, "title": title, "link": link, "summary": re.sub(r"\s+", " ", summary)[:1400], "published": pub, "weight": weight})
    return out


def score(item):
    t = (item["title"] + " " + item["summary"]).lower()
    return item["weight"] * 10 + sum(v for k, v in KEYWORDS.items() if k in t) + (1 if item["published"] else 0)
...
def call_gemini(items):
    ...
    sources = "\n".join(
        f"- {x['source']} | {x['title']} | {x['link']} | {x['summary']}"
        for x in items[:18]
    )
...
- each article must be 1400-2000 words
- body_html may use only p,h2,h3,ul,ol,pre,strong,code elements
- include at least one defensive detection example using pseudocode/SIEM logic in a pre block
- include a short "What defenders should do now" section
- end with "Sources & further reading"
- keywords: 8-12 specific search terms
...
def article_page(post, date):
    ...
    sources = "".join(
        f'<li class="source"><a href="{esc(s["url"])}" rel="noopener noreferrer">{esc(s["title"])}</a></li>'
...
def render_card(post, date):
    slug = safe_slug(post["slug"])
    return f'''<article class="post-card"><div class="post-main"><span>{date:%d %b %Y} · {esc(post["category"]).upper()}</span><h2>{esc(post["title"])}</h2><p>{esc(post["excerpt"])}</p><div class="post-tags">{" · ".join(esc(k) for k in post.get("keywords", [])[:4])}</div></div><a class="post-link" href="{date:%Y/%m/%d}/{slug}.html">READ ANALYSIS <b>↗</b></a></article>'''


def update_index(posts, date):
    idx = ROOT / "blog/index.html"
    text = idx.read_text(encoding="utf-8")
    cards = "".join(render_card(p, date) for p in posts)
    replacement = "<!-- POSTS_START -->" + cards + "<!-- POSTS_END -->"
    text = re.sub(r"<!-- POSTS_START -->.*?<!-- POSTS_END -->", replacement, text, count=1, flags=re.S)
...
def update_sitemap(posts, date):
    ...
    additions = "".join(
        f'<url><loc>{BASE}/blog/{date:%Y/%m/%d}/{safe_slug(p["slug"])}.html</loc><lastmod>{date.isoformat()}</lastmod></url>'
        for p in posts
    )
...
def main():
    ...
    result = call_gemini(candidates[:18])
    posts = result.get("posts", [])
    if len(posts) != 3:
        raise RuntimeError(f"Gemini returned {len(posts)} posts; exactly 3 are required")

    used = set()
    for post in posts:
        slug = safe_slug(post.get("slug", post.get("title", "")))
        if not slug or slug in used:
            raise RuntimeError("Gemini returned duplicate/invalid article slugs")
        used.add(slug)
        post["slug"] = slug

        out = ROOT / "blog" / f"{today:%Y}" / f"{today:%m}" / f"{today:%d}"
        out.mkdir(parents=True, exist_ok=True)
        (out / f"{slug}.html").write_text(article_page(post, today), encoding="utf-8")

    update_index(posts, today)
