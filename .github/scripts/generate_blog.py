# Gemini-powered daily Cylatic research generator: one run creates three distinct articles.
...
FEEDS = [("Google News Cybersecurity", "https://news.google.com/rss/search?q=" + urllib.parse.quote("cybersecurity when:2d") + "&hl=en-US&gl=US&ceid=US:en", 1.35),("The Hacker News", "https://feeds.feedburner.com/TheHackersNews", 1.25),("BleepingComputer", "https://www.bleepingcomputer.com/feed/", 1.25),("SecurityWeek", "https://feeds.feedburner.com/securityweek", 1.10)]
...
Rules: exactly 3 posts; unique slugs; each article 1400-2000 words; body_html may use only p,h2,h3,ul,ol,pre,strong,code; include at least one defensive detection example in a pre block; include a short "What defenders should do now" section; keywords 8-12 specific search terms.
RECENT SOURCES:\n{sources}'''
...
def main():
    ...
    items.sort(key=score,reverse=True);existing="\n".join(p.read_text(errors="ignore") for p in (ROOT/"blog").rglob("*.html"));candidates=[x for x in items if x["title"][:90].lower() not in existing.lower()]
    ...
    for post in posts:
        ...
        used.add(slug);post["slug"]=slug;(out/f"{slug}.html").write_text(article_page(post,today),encoding="utf-8")
    update_index(posts,today);update_sitemap(posts,today);print("Published 3 cybersecurity research articles:");[print(f" - {p['slug']}") for p in posts]
